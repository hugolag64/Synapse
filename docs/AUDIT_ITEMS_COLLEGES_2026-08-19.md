...
**Quatrième défaut, trouvé en vérifiant en direct sur le site.** Une fois les items rattrapés dotés d'un score et d'une tâche de consolidation réelle (item 152 : échéance 2026-08-03, 17j de retard, confirmée sur sa fiche détail), `/colleges` et `/items` continuaient d'afficher « à jour »/« — » pour ces mêmes items — la tâche n'existait que sur la fiche individuelle. Cause : `_compute()` (Items comme Collèges) n'assemblait que `review_service.generate_reviews()` (cycle J classique) et n'a jamais appelé `get_due_consolidation_tasks()` — les deux systèmes de tâches vivent en parallèle depuis la conception de la consolidation (17 juillet) sans jamais avoir été raccordés. Fusion ajoutée dans les deux `_compute()` (`all_tasks + get_due_consolidation_tasks(context="college")`).

Une fois la fusion posée, la tâche restait pourtant invisible en vérification directe : `get_due_consolidation_tasks()` retournait **0** tâche alors que l'appel direct pour ce cours seul en trouvait une. Cause racine : la fonction appliquait `filter_active_review_tasks(tasks, get_study_resume_date(...))` — le filtre de « reprise » ajouté au N01/N02 pour cacher le passif accumulé pendant une pause d'étude (cycle J classique). Sa valeur par défaut (`study_resume_date`, réglable dans Paramètres) est un littéral codé en dur au jour où N01/N02 a été écrit (`2026-08-20`) — qui se trouve être *aujourd'hui*. Une tâche de consolidation est amorcée (bootstrap) à la volée : sa `due_date` peut retomber avant cette frontière dès sa création si l'item est connu depuis longtemps — ce n'est pas du passif de pause, c'est le signal même qu'il faut réviser maintenant. `get_due_consolidation_task_for_course` (fiche détail, singulier) n'appliquait déjà pas ce filtre — la même tâche existait ou non selon la vue. **Correctif** : le filtre de reprise est retiré de `get_due_consolidation_tasks()`, aligné sur le comportement de la fiche détail. Vérifié en direct après redémarrage : Cardiovasculaire ❤️ (item 152 inclus) passe de « à jour » à **25 en retard** ; 113 tâches de consolidation dues au total contre 0 avant correctif.

**Critères d'acceptation — vérifiés.** `test_consolidation_tasks_are_merged_into_the_list_view`, `test_colleges_view_merges_consolidation_tasks_too`, `test_consolidation_tasks_are_not_hidden_by_the_reentry_boundary` (remplace `test_consolidation_reads_the_reentry_boundary`, qui verrouillait le défaut). Suite complète : 1731 tests passent, 1 échec antérieur et sans rapport. Vérifié en direct sur `/colleges` et `/items` après redémarrage du serveur.

---

*Audit réalisé sur `main@8124646`, le 19 août 2026. Toutes les mesures reflètent l'état de `data/synapse_local.db` à cette date. Les cinq défauts S1 (N00 à N03, N08, N10) ont été corrigés le jour même ; les Lots 2 à 5, N14/N15 (Q2/Q3) et les quatre défauts post-audit de consolidation (collège validé, point d'entrée unique, bouton Commencer/Planifier, tâches de consolidation invisibles en vue liste) ont été traités le 20 août. Les 21 constats de l'audit sont clos (N15 retracté — ce n'était pas un défaut). Suite finale : 1731 tests passent, 1 échec antérieur et sans rapport.*
