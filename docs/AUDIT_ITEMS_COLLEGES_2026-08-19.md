...
| N15 | 56 % d'items « indispensables » | S3 | Algorithmes | ✅ |
...
- **N15 (Q3).** `_priority_from_session_count` (`backend/core/ednpro/frequency.py`) classait 205 items sur 367 (56 %) en « indispensable » via un seuil fixe (`session_count >= 3`) qui ne s'adapte pas au volume du corpus. `normalize_training_payload` classe maintenant par **quartile** de `session_count × question_count` (nouvelle fonction `_priorities_by_quartile`) les items sans priorité explicite dans le payload EDNpro — avec repli sur l'ancien seuil fixe si le lot est trop petit (< 8 items à fréquence non nulle) pour qu'un quartile ait un sens statistique. **Appliqué à la base réelle** après confirmation : répartition passée de `indispensable 205 · important 57 · basique 67 · jamais_tombé 38` à `indispensable 82 · important 83 · basique 102 · jamais_tombé 100` (241 items reclassés), via le chemin d'écriture existant (`replace_ednpro_item_frequencies`, qui archive un instantané dans `ednpro_frequency_history` à chaque écriture). *Note de transparence* : l'archivage automatique de l'instantané précédent ne s'est pas déclenché ici (il ne s'active que si la table d'historique est vide, ce qui n'était pas le cas) — l'ancienne répartition n'est donc pas rejouable depuis l'historique, mais elle reste entièrement reconstructible : `session_count`/`question_count` n'ont pas changé, et l'ancienne priorité n'était qu'une fonction pure de `session_count`.

**Critères d'acceptation — vérifiés.** `test_consolidation_evidence_can_replace_the_literal_j_cycle`, `test_priority_without_an_explicit_value_uses_quartiles_on_a_large_batch`, `test_priority_falls_back_to_fixed_thresholds_below_the_quartile_minimum`. Suite complète : 1716 tests passent, 1 échec antérieur et sans rapport.
...
**Q3 — Comment hiérarchiser la priorité annale ?** 205 items sur 367 « indispensables ». Options : (a) relever les seuils ; (b) quartiles sur `session_count × question_count` ; (c) n'afficher un badge que dans le quartile supérieur.
