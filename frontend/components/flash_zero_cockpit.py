"""Wizard Flash-Zero (« Pièges éliminatoires ») du cockpit NiceGUI.

La carte d'accueil vit désormais dans `daily_training.py`, qui regroupe les
deux entraînements quotidiens sous un même bloc.
"""
        ...
        def draw() -> None:
            ...
            with body:
                if state["index"] >= len(questions):
                    ui.label("Pièges éliminatoires terminés").classes("text-lg font-semibold")
                ...
                with ui.column().classes("w-full gap-0 p-5"):
                    with ui.row().classes("w-full items-center justify-between"):
                        ui.label("Pièges éliminatoires · 5 min").classes("text-lg font-semibold")
